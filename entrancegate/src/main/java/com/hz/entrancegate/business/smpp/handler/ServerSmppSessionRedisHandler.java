...
import com.hz.entrancegate.business.pojo.MsgVo;
import com.hz.entrancegate.business.pojo.SmppUserVo;
import com.hz.entrancegate.business.utils.PduUtils;
import com.hz.entrancegate.business.utils.RedisUtil;
...
public class ServerSmppSessionRedisHandler extends DefaultSmppSessionHandler {
	...
	public void putSelfQueue(SubmitSm submitSm, int type) {
		...
		//营销
		if (ServerInit.CHANNEL_YX_LIST.contains(sessionKey)) {
			if (type == 1) {
			...
			//通知
		} else if (ServerInit.CHANNEL_TZ_LIST.contains(sessionKey)) {
			if (type == 1) {
			...
			//opt  验证码
		} else if (ServerInit.CHANNEL_OPT_LIST.contains(sessionKey)) {
			if (type == 1) {
