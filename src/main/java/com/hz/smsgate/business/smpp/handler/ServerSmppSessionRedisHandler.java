...
public class ServerSmppSessionRedisHandler extends DefaultSmppSessionHandler {
    ...
    public void putSelfQueue(SubmitSm submitSm, int type) {
        ...
        if (ClientInit.CHANNEL_YX_LIST.contains(sessionKey)) {
            if (type == 1) {
                byte[] shortMessage = submitSm.getShortMessage();
                int msgLen = shortMessage[4];
                if (msgLen >= 10) {
                    serverSmppSessionRedisHandler.redisUtil.lPush(SmppServerConstants.WEB_LONG_LONG_SUBMIT_SM_YX, submitSm);
                } else {
            ...
            if (type == 1) {
                if (StaticValue.SYSTEMID_CM_3.equals(submitSm.getSystemId())) {
                ...
                if (StaticValue.SYSTEMID_CM_3.equals(submitSm.getSystemId())) {
                    serverSmppSessionRedisHandler.redisUtil.lPush(SmppServerConstants.WEB_CM_SUBMIT_SM_OPT, submitSm);
                } else {
