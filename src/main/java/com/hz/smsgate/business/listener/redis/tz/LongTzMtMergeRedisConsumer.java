 ...
 */
@Component
public class LongTzMtMergeRedisConsumer implements Runnable {
    private static Logger LOGGER = LoggerFactory.getLogger(LongTzMtMergeRedisConsumer.class);

    @Autowired
    public RedisUtil redisUtil;

    public static LongTzMtMergeRedisConsumer longMtMergeRedisConsumer;
    ...
    private static final Map<String, Integer> TZ_MERGE_SIZE_MAP = new LinkedHashMap<>();
        ...
        while (true) {
            ...
            try {
                if (longMtMergeRedisConsumer.redisUtil != null) {
                    obj = longMtMergeRedisConsumer.redisUtil.rPop(SmppServerConstants.WEB_LONG_SUBMIT_SM_TZ);
                    if (obj != null) {
    ...
    public void mergeSt() throws Exception {
        ...
        for (Map.Entry<String, SubmitSm> entry : CACHE_MAP.entrySet()) {
            ...
            if ("1".equals(key.substring(key.length() - 1)) && flag) {
                Integer size = TZ_MERGE_SIZE_MAP.get(entry.getKey());
                if (size != null && size > 0) {
                    size++;
                } else {
                    size = 1;
                }
                TZ_MERGE_SIZE_MAP.put(entry.getKey(), size);
                if (size > 11) {
                    continue;
                }
                ...
                if (!StringUtils.isBlank(tempKey) && key.substring(0, key.length() - 1).equals(tempKey.substring(0, tempKey.length() - 1))) {
                    ...
                    if (completeMap.size() == Integer.valueOf(msgCount)) {
                        ...
                        if (submitSm != null) {
                            TZ_MERGE_SIZE_MAP.remove(tempKey);
                            tempKey = "";
                            flag = true;
                            try {
                                longMtMergeRedisConsumer.redisUtil.lPush(SmppServerConstants.WEB_LONG_SUBMIT_SM_SEND_TZ, submitSm);
                            } catch (Exception e) {
        ...
        LOGGER.info("TZ_MERGE_SIZE_MAP的size为{}",TZ_MERGE_SIZE_MAP.size());
