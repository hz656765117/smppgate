 ...
 */
@Component
public class LongOptRealMtSendRedisConsumer implements Runnable {
	private static Logger LOGGER = LoggerFactory.getLogger(LongOptRealMtSendRedisConsumer.class);

	@Autowired
	public RedisUtil redisUtil;

	public static LongOptRealMtSendRedisConsumer longOptRealMtSendRedisConsumer;
	...
	public void run() {
		...
		LOGGER.info("{}-长短信（redis）OPT真实发送线程开始工作......", Thread.currentThread().getName());
		SubmitSm submitSm;
		...
		while (true) {
			...
			try {
				...
				obj = longOptRealMtSendRedisConsumer.redisUtil.rPop(SmppServerConstants.WEB_REL_LONG_SUBMIT_SM_SEND_OPT);
				if (obj == null) {
					//三个通道都没消息 则休眠1s
					Thread.sleep(1000);
					continue;
