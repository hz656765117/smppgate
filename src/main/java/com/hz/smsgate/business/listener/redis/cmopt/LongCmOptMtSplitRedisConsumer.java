 ...
 */
@Component
public class LongCmOptMtSplitRedisConsumer implements Runnable {
    private static Logger LOGGER = LoggerFactory.getLogger(LongCmOptMtSplitRedisConsumer.class);


    @Autowired
    public RedisUtil redisUtil;

    public static LongCmOptMtSplitRedisConsumer longMtSplitRedisConsumer;
    ...
    public void run() {
        ...
        SubmitSm submitSm;
        Object obj;
        LOGGER.info("{}-处理将CM长短信拆分并放到真正的发送队列中(OPT)线程（redis）开始工作......", Thread.currentThread().getName());
        ...
        while (true) {
            ...
            try {
                if (longMtSplitRedisConsumer.redisUtil != null) {
                    obj = longMtSplitRedisConsumer.redisUtil.rPop(SmppServerConstants.WEB_LONG_CM_SUBMIT_SM_SEND_OPT);
                    if (obj != null) {
    ...
    public static void splitSubmitSm(SubmitSm submitSm) {
        ...
        try {
            ...
            //少于255个字符 不拆分短信
            if (msgLen < 255) {
                longMtSplitRedisConsumer.redisUtil.lPush(SmppServerConstants.WEB_REL_CM_LONG_SUBMIT_SM_SEND_OPT, submitSm);
                return;
            }
