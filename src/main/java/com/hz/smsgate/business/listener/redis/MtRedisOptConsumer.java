 ...
 */
@Component
public class MtRedisOptConsumer implements Runnable {
	private static Logger LOGGER = LoggerFactory.getLogger(MtRedisOptConsumer.class);

	@Autowired
	public RedisUtil redisUtil;

	public static MtRedisOptConsumer mtRedisOptConsumer;
	...
	public void run() {
		...
		LOGGER.info("{}-处理短信（redis）OPT下行线程开始工作......", Thread.currentThread().getName());
		...
		while (true) {
			try {
				...
				obj = mtRedisOptConsumer.redisUtil.rPop(SmppServerConstants.WEB_SUBMIT_SM_OPT);
...
//				mtRedisOptConsumer.redisUtil.hmRemove(SmppServerConstants.WEB_MSGID_CACHE, submitSm.getTempMsgId());
	...
	private void handleMsgId(SubmitSmResp submitResp, String msgId,SubmitSm submitSm) {
		...
		try {
			...
			mtRedisOptConsumer.redisUtil.lPush(SmppServerConstants.UPDATE_SUBMIT_SM, new MsgRelateVo(msgId, messageId));
